  ...
  if($_SERVER['REQUEST_METHOD']=='POST')
{
  ...
  $Query = "UPDATE `markets` SET `name`='$name',`details`='$details',`phone`='$phone',`link`='$link' where `market_id` = $id";

  if(mysqli_query($conn, $Query))
  {
    header("Location: market.php?status=Updated");
  }
  else
  {
    ?>
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <?php
              $select="SELECT * FROM `markets` WHERE `market_id` =" .$id. " LIMIT 1";
