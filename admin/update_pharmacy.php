  ...
  if($_SERVER['REQUEST_METHOD']=='POST')
{
  ...
  $Query = "UPDATE `pharmacies` SET `name`='$name',`details`='$details',`phone`='$phone',`link`='$link' where `pharmacy_id` = $id";

  if(mysqli_query($conn, $Query))
  {
    header("Location: pharmacies.php?status=Updated");
  }
  else
  {
    ?>
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <?php
              $select="SELECT * FROM `pharmacies` WHERE `pharmacy_id` =" .$id. " LIMIT 1";
