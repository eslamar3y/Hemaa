  ...
  if($_SERVER['REQUEST_METHOD']=='POST')
{
  ...
  $Query = "UPDATE `restaurants` SET `name`='$name',`details`='$details',`phone`='$phone',`link`='$link' where `restaurant_id` = $id";

  if(mysqli_query($conn, $Query))
  {
    header("Location: restaurants.php?status=Updated");
  }
  else
  {
    ?>
      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <?php
              $select="SELECT * FROM `restaurants` WHERE `restaurant_id` =" .$id. " LIMIT 1";
